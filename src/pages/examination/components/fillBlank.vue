<template>
  <div class="question-detail">
    ...
    <div v-for="(item, index) in vacancyArray" :key="index" >
      <BlankItem :showInput="showCoverView" :index="index" :value="vacancyArray[index]" @change="handleChange(index, $event)"/>
    </div>
    <div class="blank"></div>
  ...
  methods: {
    handleChange (index, value) {
      ...
      this.$emit('change', {
        ...
        selected: this.vacancyArray.some(item => item.length > 0)
      })
    },
    nextQuestion () {
      this.$emit('next')
